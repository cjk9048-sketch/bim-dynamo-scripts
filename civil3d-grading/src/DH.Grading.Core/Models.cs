...
public sealed class SlopeZone
{
    ...
    /// <summary>★★★[JACK 0915] <b>부분 지정으로 만든 구간인가.</b>
    ///
    /// <para>JACK: <i>"옹벽변환 전체구간으로 했는데 … <b>구간변환만 손댄 거잖아.
    /// 전체구간변환기능은 이전이랑 동일해야 해.</b>"</i></para>
    ///
    /// <para>0914에 <b>옹벽 구간을 정지면에서 떼는</b> 길을 냈는데(실험 중인 전이면 2단계),
    /// 그때 <b>부분인지 전체인지를 안 가렸다</b> — 그래서 <b>선 전체를 옹벽으로 바꾸는</b>
    /// 원래 기능까지 그 실험 길로 끌려 들어갔다. 쓰던 기능이 바뀐 것이다.</para>
    ///
    /// <para>부분/전체는 <see cref="Commands"/> 쪽 <c>ZoneEditCommon</c>이 <b>이미 알고 있었지만</b>
    /// 구간에 안 담아 보내 아래쪽에서는 알 길이 없었다 — 그 한 칸이 이것이다.
    /// <b>기본값 <c>false</c>(전체)</b>라 옛 번들에서 올라온 구간은 <b>종전대로</b> 돈다.</para></summary>
