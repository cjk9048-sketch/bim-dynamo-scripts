...
public sealed class StrataModel
{
    ...
    /// <para>두께처럼 지형을 따라가게 하면 언덕 위 물이 언덕만큼 올라간다 — 물은 그렇지 않다.</para>
    ///
    /// <para>★★★[JACK 0907 확인] <i>"모든 암층과 지하수위를 두께로 다 똑같이 넣었는데
    /// 왜 지하수위가 원지반과 평행하지 않지?"</i> — <b>평행하지 않는 것이 맞다.</b>
    /// <b>진짜 차이는 선택지의 유무다</b>(0907에 JACK이 짚어 주기 전까지 나도 틀리게 알고 있었다):
    /// <list type="bullet">
    /// <item>지층은 층마다 <see cref="InterpMode"/>로 <b>두께/표고를 고른다</b> —
    ///   두께로 넣으면 <c>top − 두께</c>라 <b>지형을 그대로 따라가 나란해진다</b>.</item>
    /// <item>지하수위는 <b>그 선택이 없다</b> — 두께로 넣어도 언제나 표고로 이어진다.</item>
    /// </list>
    /// 실측(0907): 지층 격자에서 지반 63.92~126.71m일 때 지하수위 63.92~118.95m —
    /// <b>낮은 쪽은 같고</b>(지표에 붙임) <b>높은 쪽은 7.76m 낮다</b>.</para>
    /// <para>JACK에게 <b>깊이로 따라가게 할지</b> 물었고 <b>지금대로 표고로</b>를 고르셨다.
    /// 수문지질 관례와도 맞다 — 지하수면은 지형을 따르되 <b>훨씬 완만하다</b>.
    /// <b>다시 묻지 말 것</b>: 이것은 결함이 아니다.
    /// (깊이로 바꾸고 싶어지면 <see cref="InterpMode"/>를 물에도 붙이면 된다 — 통로는 이미 있다.)</para></summary>
