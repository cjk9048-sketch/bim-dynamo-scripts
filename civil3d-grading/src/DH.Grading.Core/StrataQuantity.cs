...
public static class StrataQuantity
{
    ...
    ///
    /// <param name="strata">층 하단 경계들 — <b>위에서 아래 차례</b>. 비어 있으면 전부 토사로 본다.</param>
        ...
        double deepLimit = 5.0)
    {
        ...
        // ── ① 모든 꺾임점을 <b>한 가로축</b>에 모은다.
        //   TIN은 삼각형 조각이라 꺾임점 사이는 진짜 직선이다 —
        //   그래서 사다리꼴 계산이 <b>근사가 아니라 정확값</b>이 된다(기존 XsecQuantity와 같은 원리).
        ...
        double[] x = XsecQuantity.Union(arrays.ToArray());
        if (x.Length < 2) return "가로축을 못 만들었다";
        ...
        if (strata != null && strata.Count > 0)
        {
            ...
            foreach (var b in strata)
            {
                ...
                double[] bot = XsecQuantity.Resample(b.X, b.Y, x);
                bands.Add((b.Rock, top, bot));
                top = bot;
            }
            // 마지막 층 <b>아래</b>는 그 층이 계속되는 것으로 본다 — 시추가 안 닿은 깊이다.
            //   ★그냥 버리면 깊은 터파기가 <b>조용히 사라진다</b>. 아주 깊은 바닥을 깔아 둔다.
            if (bands.Count > 0)
                bands.Add((bands[bands.Count - 1].Rock, top, Fill(x.Length, -1e6)));
        }
        else
        {
        ...
        double[] surf = new double[x.Length];
        for (int i = 0; i < x.Length; i++) surf[i] = P == null ? G[i] : Math.Min(G[i], P[i]);
