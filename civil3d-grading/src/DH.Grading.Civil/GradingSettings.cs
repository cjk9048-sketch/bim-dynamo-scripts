...
/// <summary>옹벽(가시설) 형태 — 정본은 <see cref="WallStyle"/>(WallStyle.cs).
/// <para>★[4단계] 예시 그림을 <b>딸린 것 없이</b> 시험할 수 있게 파일을 갈랐다 —
/// 이 파일은 레지스트리·도면을 읽어 오프라인에서 못 돌린다.</para></summary>
...
public static class GradingSettings
{
    ...
    /// <summary>★[§78 규칙] <b>설치하는 순간에만</b> 올린다 — 작업 중에는 안 건드린다.
    /// <para>v72.0은 대수술 2단계에서 찍혔고 그 뒤 세 번 반려되며 치명 여섯이 더 나왔다.
    ...
    /// </list>
    ///
    /// <para>★★★[JACK 0916 <i>"자, 다시 처음으로 돌려. 일단 노선을 선정하면 <b>가상 옹벽만</b> 나오게 롤백해봐"</i>]
    /// <b>지금 1이다.</b></para>
    ///
    /// <para><b>왜 되돌렸나 — 숫자로.</b> 2단계(자르고 합성)가 뚜껑을 깨뜨렸다.
    /// 0916 마지막 판 실측: 벽 되읽기 <b>0.000m</b>(완벽)인데 뚜껑 <b>3.696m</b> · 어긋남 <b>62/120</b>.
    /// 그리고 JACK이 화면에서 본 것 — <i>"평면부에서 보면 데이라잇하고 안 맞어, 또 튀어나와"</i>.</para>
    ///
    /// <para><b>뿌리는 데이라잇을 따는 법</b>이다. 같은 판 <c>②칼금 품질</c> 실측:
    /// 옹벽∩원지반 <b>진짜 74%</b>(가짜 ≈73m · 최악 10.17m) ·
    /// 옹벽∩정지면 <b>진짜 71%/70%</b>(가짜 ≈27m/24m · 최악 <b>15.00m</b>).
    /// <b>넷 중 하나가 가짜</b>고, 가장 긴 가짜 한 토막이 15m다.
    /// 그 선으로 자르니 자를 때마다 엉뚱한 데가 잘렸다.</para>
    ///
    /// <para>→ <b>2·3단계 코드는 지우지 않았다.</b> 데이라잇을 제대로 따게 되면 이 숫자만 도로 올린다.</para></summary>
    public static int TransitionStage = 1;

    /// <summary>★★★[JACK 0916 <i>"빨간색으로 화살표랑 경로 나오고 <b>누르면 폴리곤만 생성</b>되는 것까지만 해봐"</i>]
    /// <b>구간을 고르면 폴리곤 하나만 그린다.</b>
    ///
    /// <para>켜면 이것만 한다 — <b>빨간 미리보기(화살표·경로)</b> → 고르면 <b>쐐기 폴리곤</b>
    /// (레이어 <c>DH-가상폴리곤</c>) 한 줄. 표면은 <b>하나도 안 만든다</b>.</para>
    ///
    /// <para><b>왜 여기까지 내렸나.</b> 자르고 합성하는 데를 열 번 넘게 고쳤는데
    /// 고칠 때마다 다른 데가 깨졌다. 마지막 판 실측이 그 모습이다 —
    /// 벽 되읽기 <b>0.000m</b>(완벽)인데 뚜껑 <b>3.696m</b> · 어긋남 <b>62/120</b>,
    /// 그리고 화면에서는 <i>"평면부에서 데이라잇하고 안 맞어, 또 튀어나와"</i>.</para>
    ///
    /// <para><b>뿌리는 데이라잇을 따는 법</b>이다. 같은 판 실측:
    /// 옹벽∩원지반 <b>진짜 74%</b> · 옹벽∩정지면 <b>71%/70%</b> —
    /// <b>넷 중 하나가 가짜</b>고 가장 긴 가짜가 <b>15m</b>다. 그 선으로 자르니 엉뚱한 데가 잘렸다.</para>
    ///
    /// <para>→ 폴리곤 하나부터 눈으로 맞히고, 그 다음에 데이라잇을 그 폴리곤에 <b>넣는다</b>.
    /// 2·3단계 코드는 <b>지우지 않고 꺼 뒀다</b>(커밋 <c>157abfe</c>에도 그대로 있다) —
    /// 데이라잇이 제대로 잡히면 도로 켜면 된다.</para></summary>
    public static bool WallPolygonOnly = true;

    /// <summary>★[JACK 0916] 폴리곤 <b>폭에 더하는 여유</b>(m).
