...
public sealed class ResetCommand
{
    ...
    ///  · includeImported=false: 정지 산출물만 지우고 가져온 등고선·지적도·'원지반'은 보존.
    ///    ★[검토 0901] <b>지금은 부르는 데가 없다</b> — JACK 0901 지시로 초기화 버튼도 true가 됐다.
    ///    지우지 않고 남겨 둔 것은 나중에 "가져온 것은 두고 정지만 되돌리기"가 필요할 때를 위해서다.
