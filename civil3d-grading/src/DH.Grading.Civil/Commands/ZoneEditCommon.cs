...
internal static class ZoneEditCommon
{
    ...
    public static void Run(Document doc, bool wallMode)
    {
        ...
        try
        {
            ...
            // ══ ★★★[JACK 0914] <b>날개벽 미리보기를 통째로 걷었다.</b> ═══════════════════
            //
            //   <para>여기서 <c>WingLine</c>으로 <b>구간 → 데이라잇 직선</b>을 긋고
            //   <c>WallFromLine</c>으로 면까지 쌓아 <c>DH-날개벽선</c>·<c>DH-날개벽면</c>에 그렸다.
            //   그 방식은 버려졌는데 <b>그리기는 계속돼</b> 화면을 어지럽혔다 —
            //   JACK: <i>"구간선정에서 데이라잇까지 <b>직선으로 3d폴리선</b>으로 작성됐어"</i>가 그것이다.</para>
            //
            //   <para>지금 옹벽은 <c>CreateGradingCommand</c>가 <b>구간(zone)에서 유도</b>해
            //   <c>가상옹벽_DH</c>로 만든다(<c>DH-가상옹벽선</c>·<c>DH-가상폴리곤</c>).</para>
                           ...
                           double[] rcum, double t0, double t1)
            {
                ...
                // ★옛 미리보기 선이 도면에 남아 있으면 걷는다(전 판에서 그린 것).
                try
                {
                    using var trW = db.TransactionManager.StartTransaction();
                    GradingBuilder.EraseOnLayer(db, trW, "DH-날개벽선");
                    GradingBuilder.EraseOnLayer(db, trW, "DH-날개벽면");
                    trW.Commit();
                }
                catch { }
                Log($"   날개벽 미리보기 — <b>걷었다</b>(구간 [{t0:F1}..{t1:F1}]). 옹벽은 적용할 때 '가상옹벽_DH'로 만들어진다.");
            }
