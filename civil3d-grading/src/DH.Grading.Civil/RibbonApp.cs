    ...
    private void BuildRibbon()
    {
        try
        {
            ...
            btnSet.ToolTip = MakeTip("계획부지 정지 창 (DHGRADEPANEL)",
                ...
                // ★[검토 0915 · 보통] <b>화면에 뜨는 글이 실제와 달랐다.</b>
                //   없어진 이름(<c>1. 대상</c>·<c>[지표면 생성]</c>)과 0910에 지운 <c>[값 저장]</c>을
                //   아직 안내하고 있었다 — 창을 처음 여는 사람이 <b>없는 단추를 찾게</b> 된다.
                "· [계획 경계 선택] → [원지반 선택] → 값을 정하고 → [계획부지생성하기]\n" +
                "· 절토/성토 제원과 예시 그림 · 산지 대소단\n" +
                "· 계획부지 수정(옹벽 변환 / 사면 변환)\n\n" +
                "※ 칸 제목(■ 계획부지생성 · ■ 계획부지 수정)을 누르면 접었다 펼칠 수 있습니다.\n" +
                "※ <b>옹벽 형태</b>는 [기타] → [기타 설정]으로 옮겼습니다.\n" +
                "※ 값은 [계획부지생성하기]를 누를 때 함께 반영됩니다(따로 저장할 것 없습니다).", null);
