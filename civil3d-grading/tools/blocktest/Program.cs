    ...
    static double MaxOutInZone(VirtualSlope v, SlopeZone z, IReadOnlyList<Point3> b, double[] bc)
    {
        ...
        foreach (var ring in v.Rings)
            foreach (var q in ring)
            {
                ...
                // ★★[검토 0910 · 보통1] 0910에 여기서 <b>구간 양 끝 5m를 빼고</b> 쟀었다.
                //   이음매를 점으로 채우던 판에서 그 점들이 최원점을 0.5m 바깥으로 밀어
                //   이 검사가 <b>간발로</b> 떨어졌기 때문이다(118.6 → 117.6, 문턱은 >1.0).
                //   <para>★그런데 재 보니 <b>제외를 켜면 채우기 전·후의 숫자가 완전히 같아졌다</b>
                //   (둘 다 118.5 → 117.0) — 즉 이 검사가 <b>그 변화를 아예 못 보게</b> 됐다.
                //   검사가 진짜를 잡았는데 그 눈을 가린 셈이다.</para>
                //   → 이음매 채우기를 되돌렸으므로 <b>제외도 같이 걷는다</b>. 이 검사는 다시 눈을 뜬다.
                double d = double.MaxValue;
...
//   느려지는 것 자체가 결함이므로 **시간을 시험으로 못 박는다**(회귀 감시).
{
    ...
    // ★★[검토 0910] <b>세 번 재서 가장 빠른 값을 쓴다.</b>
    //   <para>이 검사가 <b>기계 사정에 따라 떨어졌다</b> — 이 기계에서 311ms로 떨어진 판이
    //   다시 돌리니 276·195·160ms였고, 검토자 기계(빌드를 같이 돌리던 중)에서는 두 건이 떨어졌다.
    //   묻는 것은 <b>"이만큼 빠를 수 있나"</b>(=코드가 느려졌나)이지 "언제나 이만큼인가"가 아니다.
    //   한 번만 재면 옆에서 도는 빌드·바이러스 검사가 <b>가짜 경보</b>를 내고,
    //   가짜 경보가 잦으면 <b>진짜 경보를 무시하게 된다</b> — 그게 더 비싸다.</para>
    long Best(List<SlopeZone>? zs)
    {
        long best = long.MaxValue;
        for (int i = 0; i < 3; i++) best = Math.Min(best, Ms(zs));
        return best;
    }
    Ms(null); Ms(null);   // 워밍업(JIT)
    long t0 = Best(null), t4 = Best(MakeZones(4)), t8 = Best(MakeZones(8)), t16 = Best(MakeZones(16));
    Console.WriteLine($"      S54 단 {nb}개 · Build 시간(3판 중 최단) — 구간 0개 {t0}ms · 4개 {t4}ms · 8개 {t8}ms · 16개 {t16}ms");
...
//   ※이 검사는 <b>고치기 전에는 반드시 실패한다</b> — 무동작으로 통과하지 않는다.
{
    ...
    // ★★[JACK 0910 · 검토 0910 실측] <b>이 재현 조건은 아직 살아 있다.</b>
    //   옹벽 구간이 있으면 링이 이음매에서 벌어진 채 조립된다(§68) — 그 벌어진 자리가 100m대라
    //   아래 <b>가짜 옹벽선 문턱</b>이 무방비가 되는 것이 이 검사의 뜻이다.
    //   <para>0910에 한 번 <b>이음매를 점으로 채워</b> 이 값을 1.0m로 내렸는데, 재 보니
    //   그것은 <b>모양을 안 바꾸고 브레이크라인 문턱만 우회</b>하는 일이었다
    //   (옹벽선 15→29줄 · 구간 밖인데 벽 0→226점). 그래서 <b>되돌렸다</b>.
    //   근원은 전이면(마감면)을 세워야 없어진다 — S106 참조.</para>
    Check("S95 ★재현 조건: 부분 구간이면 링이 이음매에서 벌어져 있다", maxGap95 > 50,
          $"최대 벌어짐 {maxGap95:F1}m (전이면을 세우면 이 값이 내려간다)");
    ...
    // ★★★[검토 0910 · 치명1] <b>여태 아무도 이것을 단언하지 않았다.</b>
    //   진단줄은 <c>⚠구간 밖인데 벽 226점</c>까지 찍어 놓고 <b>PASS로 지나갔다</b>.
    //   그 226점이 0910 이음매 채우기가 부른 회귀였는데, 검사기가 인쇄만 해서 못 잡았다.
    //   → 이제 <b>수로 단언한다</b>(<c>WallRunBuilder.LastExtraWall</c>).
    //     0이 아니면 "규칙은 사면인데 기하는 벽" — 0805 사선 옹벽이 돌아온 것이다.
    Check("S95 ★★★구간 밖에 옹벽이 서지 않는다(사선 옹벽 되돌아옴 감시)",
        WallRunBuilder.LastExtraWall == 0,
        $"구간 밖인데 벽 {WallRunBuilder.LastExtraWall}점 (0이어야 한다)");
}
...
// ══ S103 구간 안의 부분 구간 — [옹벽 변환]에서 시점·종점을 찍는 셈 (JACK 0910) ══
//   화면은 못 시험하지만 <b>셈은 여기서 다 시험한다</b> — 이 저장소가 §78 ③에서 값을 치르고 정한 규칙.
{
    // 40×40 사각 링, 둘레 160. 꼭짓점 t = 0/40/80/120.
    var ring = new List<Point3> { new(0,0,0), new(40,0,0), new(40,40,0), new(0,40,0) };
    var cum = GradingGeometry.CumLen2D(ring);
    double total = cum[cum.Length - 1];
    Check("S103 둘레가 160이다", System.Math.Abs(total - 160) < 1e-6, $"{total:0.###}");

    // ① 안/밖 판정 — 보통 구간과 0을 지나는 구간
    Check("S103 보통 구간: 안에 있다", GradingGeometry.InInterval(10, 50, 30), "");
    Check("S103 보통 구간: 밖에 있다", !GradingGeometry.InInterval(10, 50, 60), "");
    Check("S103 ★랩 구간: 0 바로 뒤가 안이다", GradingGeometry.InInterval(150, 10, 5), "");
    Check("S103 ★랩 구간: 가운데는 밖이다", !GradingGeometry.InInterval(150, 10, 80), "");

    // ② 가두기 — 구간 밖을 찍으면 가까운 끝으로 붙는다
    double t = GradingGeometry.ClampInto(10, 50, 30, total, out double mv);
    Check("S103 구간 안이면 그대로 둔다", System.Math.Abs(t - 30) < 1e-9 && mv == 0, $"{t:0.##}");
    t = GradingGeometry.ClampInto(10, 50, 55, total, out mv);
    Check("S103 ★구간 밖은 가까운 끝(50)으로 붙는다", System.Math.Abs(t - 50) < 1e-9 && System.Math.Abs(mv - 5) < 1e-9, $"{t:0.##} · {mv:0.##}m");
    t = GradingGeometry.ClampInto(10, 50, 5, total, out mv);
    Check("S103 ★반대쪽 밖은 시점(10)으로 붙는다", System.Math.Abs(t - 10) < 1e-9 && System.Math.Abs(mv - 5) < 1e-9, $"{t:0.##} · {mv:0.##}m");
    t = GradingGeometry.ClampInto(150, 10, 140, total, out mv);
    Check("S103 ★랩 구간 밖도 가까운 끝(150)으로", System.Math.Abs(t - 150) < 1e-9, $"{t:0.##}");

    // ③ 조각 떠내기 — 한 변 안, 꼭짓점을 지나는 것, 0을 지나는 것
    var seg = GradingGeometry.SubPath(ring, cum, 10, 30);
    double SegLen(List<Point3> v)
    {
        double L = 0;
        for (int i = 1; i < v.Count; i++) { double dx = v[i].X - v[i-1].X, dy = v[i].Y - v[i-1].Y; L += System.Math.Sqrt(dx*dx + dy*dy); }
        return L;
    }
    Check("S103 한 변 안 조각은 두 점이다", seg.Count == 2, $"{seg.Count}점");
    Check("S103 그 길이가 20m다", System.Math.Abs(SegLen(seg) - 20) < 1e-6, $"{SegLen(seg):0.###}m");

    seg = GradingGeometry.SubPath(ring, cum, 30, 50);
    Check("S103 ★꼭짓점을 지나면 그 점이 낀다(3점)", seg.Count == 3, $"{seg.Count}점");
    Check("S103 그 길이도 20m다", System.Math.Abs(SegLen(seg) - 20) < 1e-6, $"{SegLen(seg):0.###}m");

    seg = GradingGeometry.SubPath(ring, cum, 150, 10);
    Check("S103 ★★0을 지나는 조각도 이어진다", seg.Count == 3, $"{seg.Count}점");
    Check("S103 ★★그 길이가 20m다(꼬이면 300m가 된다)", System.Math.Abs(SegLen(seg) - 20) < 1e-6, $"{SegLen(seg):0.###}m");

    seg = GradingGeometry.SubPath(ring, cum, 0, 160);
    Check("S103 둘레 전체는 한 바퀴가 된다", System.Math.Abs(SegLen(seg) - 160) < 1e-6, $"{SegLen(seg):0.###}m");

    // ④ 찍은 점 → 호길이 → 다시 점 (실제로 쓰는 길)
    double tp = GradingGeometry.ParamAt(ring, cum, 25, 0.4);   // 아랫변 위 25m 자리를 살짝 벗어나 찍는다
    Check("S103 ★찍은 자리가 호길이 25m로 읽힌다", System.Math.Abs(tp - 25) < 0.5, $"{tp:0.###}m");
    var back = GradingGeometry.PointAtParam(ring, cum, tp);
    Check("S103 그 호길이의 좌표가 원래 자리 근처다",
          System.Math.Abs(back.X - 25) < 0.5 && System.Math.Abs(back.Y) < 1e-6, $"({back.X:0.##},{back.Y:0.##})");
}

// ══ S104 부분 구간 판정 — <b>명령이 실제로 부르는 그 함수</b>를 그대로 먹인다 (JACK 0910) ══
//   ★S103은 조각 셈만 시험했다. 판정(가두기·방향·거절)은 명령 안에 있어 검사기가 못 닿았고,
//     그래서 <b>닫힌 고리에서 언제나 거절되는 치명 결함</b>이 검사를 통째로 빠져나갔다(검토 0910).
//     → 판정을 Core로 내리고 여기서 <b>출하되는 그 판정</b>을 시험한다(§78 ③·④의 교훈).
{
    const double TOT = 160.0, MIN = 3.0;
    (double T0, double T1, double Span, bool Whole)? P(double f0, double f1, double a, double b, bool flip,
                                                       out double mv, out string why)
        => GradingGeometry.PartInterval(f0, f1, a, b, TOT, MIN, flip, out mv, out why, out _, out _);

    (double T0, double T1, double Span, bool Whole)? PF(double f0, double f1, double a, double b, bool flip,
                                                        out GradingGeometry.PartFail fail)
        => GradingGeometry.PartInterval(f0, f1, a, b, TOT, MIN, flip, out _, out _, out fail, out _);

    // ① 조각 구간 [10,50]
    var r = P(10, 50, 20, 40, false, out double mv, out string why);
    Check("S104 조각 구간: 두 점 사이 20m", r != null && System.Math.Abs(r.Value.Span - 20) < 1e-6, r == null ? why : $"{r.Value.Span:0.##}m");
    Check("S104 조각 구간: 한 바퀴가 아니다", r != null && !r.Value.Whole, "");
    var r2 = P(10, 50, 40, 20, false, out _, out _);
    Check("S104 ★찍는 차례를 바꿔도 같다", r2 != null && System.Math.Abs(r2.Value.Span - 20) < 1e-6
          && System.Math.Abs(r2.Value.T0 - r.Value.T0) < 1e-6, "");

    r = P(10, 50, 30, 60, false, out mv, out why);
    Check("S104 ★한 점이 구간 밖이면 끝(50)으로 붙는다",
          r != null && System.Math.Abs(r.Value.T1 - 50) < 1e-6 && System.Math.Abs(mv - 10) < 1e-6, $"{mv:0.#}m 붙임");

    r = P(10, 50, 60, 70, false, out mv, out why);
    Check("S104 ★★두 점이 다 밖이면 거절하고 <b>그 이유</b>를 말한다",
          r == null && why.Contains("구간 밖"), why);

    r = P(10, 50, 20, 21.5, false, out mv, out why);
    Check("S104 너무 가까우면 거절한다", r == null && why.Contains("가깝"), why);

    r = P(10, 50, 10, 50, false, out mv, out why);
    Check("S104 구간 전체를 찍으면 '푼다'고 말한다", r == null && why.Contains("구간 전체"), why);

    // ② ★★★한 바퀴 고리 [0,160] — 검토 0910 치명1이 살던 자리
    r = P(0, TOT, 20, 60, false, out mv, out why);
    Check("S104 ★★★닫힌 고리에서도 부분 지정이 된다(치명1)",
          r != null && System.Math.Abs(r.Value.Span - 40) < 1e-6, r == null ? why : $"{r.Value.Span:0.##}m");
    Check("S104 ★★★그때 '한 바퀴'라고 알려 준다(방향을 물어야 하므로)", r != null && r.Value.Whole, "");

    r = P(0, TOT, 5, 155, false, out mv, out why);
    Check("S104 ★★한 바퀴: 짧은 쪽(0을 지나는 10m)을 고른다",
          r != null && System.Math.Abs(r.Value.Span - 10) < 1e-6, r == null ? why : $"{r.Value.Span:0.##}m");
    Check("S104 ★★그 조각은 155 → 5로 흐른다",
          r != null && System.Math.Abs(r.Value.T0 - 155) < 1e-6 && System.Math.Abs(r.Value.T1 - 5) < 1e-6,
          r == null ? "" : $"{r.Value.T0:0.#}→{r.Value.T1:0.#}");

    r = P(0, TOT, 5, 155, true, out mv, out why);
    Check("S104 ★★반대쪽(R)을 고르면 150m가 된다",
          r != null && System.Math.Abs(r.Value.Span - 150) < 1e-6, r == null ? why : $"{r.Value.Span:0.##}m");

    r = P(0, TOT, 0, 80, false, out mv, out why);
    Check("S104 한 바퀴: 정반대 두 점은 어느 쪽이든 80m", r != null && System.Math.Abs(r.Value.Span - 80) < 1e-6,
          r == null ? why : $"{r.Value.Span:0.##}m");

    // ③ 랩 구간 [150,10] — 0을 지나는 조각 구간
    r = P(150, 10, 155, 5, false, out mv, out why);
    Check("S104 ★★랩 구간 안의 부분 지정(10m)", r != null && System.Math.Abs(r.Value.Span - 10) < 1e-6,
          r == null ? why : $"{r.Value.Span:0.##}m");
    r = P(150, 10, 80, 5, false, out mv, out why);
    Check("S104 ★랩 구간 밖을 찍으면 가까운 끝으로 붙는다", r != null && mv > 1, $"{mv:0.#}m 붙임");

    // ④ 최소 길이는 <b>실제로 쓰는 값</b>이어야 한다
    // ★<b>조각 구간</b>에서 잰다 — 한 바퀴 고리는 짧은 쪽이 모자라면 <b>반대쪽</b>을 고르는 것이 맞다(아래 ⑤).
    Check("S104 ★2m 조각은 거절된다(링 정점이 모자라 안 먹는다)",
          P(10, 50, 20, 22, false, out _, out string w2) == null && w2.Contains("가깝"), w2);
    Check("S104 3.5m 조각은 통과한다",
          P(10, 50, 20, 23.5, false, out _, out _) != null, "");

    // ⑤ ★[검토 0910 2차] 규약이 저장소와 어긋나던 자리 셋
    //   ①한 바퀴가 <c>T0 == T1</c>로 나오면 "전체"라고 적고 실제로는 <b>0</b>이 된다
    //     (<c>SlopeZone.Contains</c>·<c>LenOf</c>는 같은 값을 <b>한 점</b>으로 읽는다).
    r = P(0, TOT, 30, 30, true, out mv, out why);
    Check("S104 ★★★한 바퀴는 언제나 (0, 둘레)로 적는다(같은 점+반대쪽)",
          r != null && System.Math.Abs(r.Value.T0) < 1e-9 && System.Math.Abs(r.Value.T1 - TOT) < 1e-6
          && System.Math.Abs(r.Value.Span - TOT) < 1e-6,
          r == null ? why : $"({r.Value.T0:0.#},{r.Value.T1:0.#}) {r.Value.Span:0.#}m");

    //   ②닫힌 고리에서 "이 2m 개구부만 빼고 나머지 전부"는 정상적인 요구다.
    r = P(0, TOT, 20, 22, false, out mv, out why);
    Check("S104 ★★짧은 쪽이 최소치 미만이면 반대쪽(158m)을 고른다",
          r != null && System.Math.Abs(r.Value.Span - 158) < 1e-6, r == null ? why : $"{r.Value.Span:0.##}m");
    Check("S104 ★그때 그렇게 했다고 말해 준다", r != null && why.Contains("반대쪽"), why);

    //   ③한 점만 밖인데 "둘 다 밖"이라고 하면 사용자를 엉뚱한 곳으로 보낸다.
    r = P(10, 50, 48, 70, false, out mv, out why);
    Check("S104 ★붙인 뒤 짧아진 것이면 그 사실을 같이 적는다",
          r == null && why.Contains("붙였더니") && why.Contains("벗어남"), why);

    //   ④거절 사유는 <b>문장이 아니라 값</b>으로 갈린다.
    PF(10, 50, 10, 50, false, out var f1);
    Check("S104 ★구간 전체는 사유가 WholeInterval", f1 == GradingGeometry.PartFail.WholeInterval, f1.ToString());
    PF(10, 50, 20, 21, false, out var f2);
    Check("S104 ★너무 짧으면 사유가 TooShort", f2 == GradingGeometry.PartFail.TooShort, f2.ToString());
    PF(0, TOT, 20, 60, false, out var f3);
    Check("S104 ★정해지면 사유가 None", f3 == GradingGeometry.PartFail.None, f3.ToString());
}

// ══ S106 구간 경계의 <b>끊김</b> — JACK 0910 "부분변환한 자리가 엄청 깨진다" 오프라인 재현 ══
//   ★현장 로그가 잰 것: 링 최장변 14.67m인데 <b>완성된 지표면의 최장 변이 55.71m</b>,
//     둘레 16칸 내밀기가 13칸은 0~4.5m인데 3칸만 17~19m.
//     → 구간 경계에서 <b>벽 끝(1m)과 사면 끝(19m)</b>이 만나는데 그 사이를 이어 주는 면이 없다.
//   ★이 검사는 <b>고치기 전에 자를 만드는 것</b>이다 — 지금은 얼마나 벌어지는지 재서 남기고,
//     고친 뒤 이 숫자가 줄어드는지로 판정한다("기하 버그는 계측부터").
{
    var pad = new List<Point3> { new(0,0,100), new(80,0,100), new(80,60,100), new(0,60,100) };
    var cum106 = GradingGeometry.CumLen2D(pad);
    double per = cum106[^1];
    var gnd = new FlatGround(140);          // 원지반 140 → 절토 40m
    ...
    static (double Max, int Over5, double AtX, double AtY) EdgeStat(IReadOnlyList<Point3> r)
    {
        double mx = 0, ax = 0, ay = 0; int over = 0;
        for (int i = 0; i < r.Count; i++)
        {
            var a = r[i]; var b = r[(i + 1) % r.Count];
            double d = System.Math.Sqrt((b.X-a.X)*(b.X-a.X) + (b.Y-a.Y)*(b.Y-a.Y));
            if (d > 5.0) over++;
            if (d > mx) { mx = d; ax = a.X; ay = a.Y; }
        }
        return (mx, over, ax, ay);
    }
    static (double Max, int Over5) WorstOf(List<List<Point3>> rings)
    {
        double mx = 0; int over = 0;
        foreach (var r in rings) { if (r.Count < 3) continue; var s = EdgeStat(r); mx = System.Math.Max(mx, s.Max); over += s.Over5; }
        return (mx, over);
    }

    // ① 구간 없음 — 전부 사면. 이것이 "깨지지 않은" 기준이다.
    var vPlain = GradingGeometry.Build(pad, gnd, par, true);
    var sPlain = WorstOf(vPlain.Rings);
    Check("S106 구간이 없으면 링 변이 짧다(기준)", sPlain.Max < 8.0,
          $"최장변 {sPlain.Max:0.##}m · 5m 초과 {sPlain.Over5}개 · 링 {vPlain.Rings.Count}개");

    // ② 둘레의 <b>일부만</b> 수직(옹벽) — JACK이 한 그것.
    var zPart = new List<SlopeZone> { SlopeZone.Wall(0.0, per * 0.35, 0, int.MaxValue, 0.01, 1.0) };
    var vPart = GradingGeometry.Build(pad, gnd, par, true, zPart);
    var sPart = WorstOf(vPart.Rings);
    // ★★★[검토 0910 · 높음2] <b>조건이 <c>true</c>였다 — 이름과 값이 정반대인 채 통과했다.</b>
    //   이름은 "긴 변이 생긴다(재현)"인데 이음매를 채운 판에서는 "최장변 1m"가 찍혔다.
    //   <b>못 떨어지는 검사는 검사가 아니다</b>(「검사 입력은 출하 입력이어야 한다」의 짝).
    //   → 이제 진짜로 단언한다: 이 결함이 살아 있는 동안 이 검사는 <b>재현 검사</b>다.
    //     전이면을 세워 근원을 없애면 이 줄이 <b>떨어져서</b> 알려 준다 — 그때 뜻을 뒤집으면 된다.
    Check("S106 ★재현: 부분 구간을 넣으면 링에 <b>긴 변</b>이 생긴다", sPart.Max > 50.0 && sPart.Over5 > 0,
          $"최장변 {sPart.Max:0.##}m · 5m 초과 {sPart.Over5}개 · 링 {vPart.Rings.Count}개"
        + $" (기준 {sPlain.Max:0.##}m)");

    // ③ 얼마나 벌어지나 — 벽 쪽과 사면 쪽의 <b>내밀기</b> 차이(현장 로그의 1m ↔ 19m에 해당).
    static double OutMax(IReadOnlyList<Point3> ring, IReadOnlyList<Point3> pad0, double[] cum0)
    {
        double mx = 0;
        foreach (var q in ring)
        {
            double t = GradingGeometry.ParamAt(pad0, cum0, q.X, q.Y);
            var p0 = GradingGeometry.PointAtParam(pad0, cum0, t);
            mx = System.Math.Max(mx, System.Math.Sqrt((q.X-p0.X)*(q.X-p0.X) + (q.Y-p0.Y)*(q.Y-p0.Y)));
        }
        return mx;
    }
    double outPlain = vPlain.Rings.Count > 0 ? OutMax(vPlain.Rings[^1], pad, cum106) : 0;
    double outPart  = vPart.Rings.Count  > 0 ? OutMax(vPart.Rings[^1],  pad, cum106) : 0;

    // ★★[검토 0910 · 높음2] 여기도 조건이 <c>true</c>였다 — 두 숫자를 <b>나란히 인쇄만</b> 했다.
    //   재야 할 것은 "전체 최대 내밀기"가 아니라 <b>구간 안과 밖의 차이</b>다.
    //   그 차이가 곧 이음매에서 벌어지는 거리이고, <b>깨짐의 크기 그 자체</b>다.
    static double OutMaxIn(IReadOnlyList<Point3> ring, IReadOnlyList<Point3> pad0, double[] cum0,
                           double zt0, double zt1, bool inside)
    {
        double mx = 0;
        foreach (var q in ring)
        {
            double t = GradingGeometry.ParamAt(pad0, cum0, q.X, q.Y);
            if (GradingGeometry.InInterval(zt0, zt1, t) != inside) continue;
            var p0 = GradingGeometry.PointAtParam(pad0, cum0, t);
            mx = System.Math.Max(mx, System.Math.Sqrt((q.X-p0.X)*(q.X-p0.X) + (q.Y-p0.Y)*(q.Y-p0.Y)));
        }
        return mx;
    }
    double inWall  = vPart.Rings.Count > 0 ? OutMaxIn(vPart.Rings[^1], pad, cum106, 0.0, per*0.35, true)  : 0;
    double inSlope = vPart.Rings.Count > 0 ? OutMaxIn(vPart.Rings[^1], pad, cum106, 0.0, per*0.35, false) : 0;
    Check("S106 ★내밀기 실측 — 구간 안(벽)은 가깝고 구간 밖(사면)은 멀다", inSlope > inWall + 20.0,
          $"구간 안 {inWall:0.#}m ↔ 구간 밖 {inSlope:0.#}m — 이 차이가 이음매에서 그대로 벌어진다"
        + $" · 전체 최대 전부사면 {outPlain:0.#}m / 부분구간 {outPart:0.#}m");

    // ④ ★<b>이음매를 무엇으로 메우고 있나.</b>
    //   ★[검토 0910 · 높음2] 종전엔 <c>TransitionFaces</c>를 셌는데 <b>엉뚱한 컬렉션</b>이었다 —
    //     그건 경계 플래토 사이의 전환띠(무늬용)이지 <b>구간 이음매</b>가 아니라, 전에도 후에도 0개다.
    //     "고치면 이 값이 바뀐다"는 전제 자체가 틀렸다.
    //   → 대신 <b>가장 긴 변에 중간점이 있는가</b>를 본다. 지금은 <b>점 두 개짜리 맨 변</b>이다.
    //     0910에 여기를 <c>dens</c> 간격 점으로 채워 봤는데, 그 점들은 전부 <b>같은 직선 위</b>라
    //     모양을 못 바꾸고 브레이크라인 문턱(2.5m)만 우회해 <b>가짜 옹벽선</b>을 불렀다(되돌림).
    //     진짜 마감은 <b>단이 진 면</b>을 세우는 일이고, 그때 이 자리의 모양이 바뀐다.
    var worstRing = vPart.Rings.Count > 0 ? vPart.Rings[^1] : new List<Point3>();
    var es = worstRing.Count >= 3 ? EdgeStat(worstRing) : (Max: 0.0, Over5: 0, AtX: 0.0, AtY: 0.0);
    Check("S106 ★이음매는 아직 <b>점 두 개짜리 맨 변</b>이다(전이면 미구현)", es.Max > 50.0,
          $"맨아랫단 최장변 {es.Max:0.##}m @({es.AtX:0.#},{es.AtY:0.#})"
...
//   찾아 그 둘레값을 쓴다. 상수로 적으면 링 만드는 규칙이 바뀔 때 검사만 조용히 낡는다.</para>
{
    ...
    static bool In112(IReadOnlyList<Point3> r, double x, double y)
    {
        bool q = false;
    ...
    foreach (var (pn, pad) in new[] { ("직사각", pad112r), ("ㄴ자", pad112n) })
    {
        ...
        foreach (var (dn, up, gz, zLine) in new[] { ("절토", true, 140.0, 105.0), ("성토", false, 60.0, 95.0) })
        {
            ...
            // ★★[검토 0911] <b>옛 데이라잇 선은 그 자리 원지반 표고를 갖는다</b>(정의상
            //   "사면이 원지반과 닿는 선"). <c>OffsetRingForTest</c>는 Z=0을 주므로 그대로 쓰면
            //   <b>출하 입력과 다른 값</b>을 먹이는 것이 된다.
            var oldDay = GradingGeometry.OffsetRingForTest(pad, 60.0, par112);
            if (oldDay != null)
                for (int i = 0; i < oldDay.Count; i++)
                    oldDay[i] = new Point3(oldDay[i].X, oldDay[i].Y, gz);

            // 구간 자리 셋 — 변 가운데 · 코너에 걸침 · 코너에서 코너까지
            var spots = new List<(string Name, double T0, double T1)>();
            // ★★[검토 0911 · 높음8h] <b>이름을 실측에 맞춘다.</b> 종전 "변 가운데"는 실은
            //   한 끝이 코너였고(아니오/예) "코너에 걸침"은 양 끝 다 코너가 아니었다(아니오/아니오).
            //   이름이 사실과 다르면 로그를 읽는 사람이 <b>엉뚱한 판을 봤다고 믿는다</b>.
            double Wr112(double t) => ((t % rt) + rt) % rt;
            spots.Add(("한 끝이 코너", rt * 0.08, rt * 0.22));
            if (corners.Count >= 2)
            {
                spots.Add(("코너를 가로지름", Wr112(corners[0] - 20.0), Wr112(corners[0] + 20.0)));
                spots.Add(("코너→코너", corners[0], corners[1]));
                // ★★★[검토 0911 · 높음7] <b>오목한 굽이를 일부러 품는 판.</b>
                //   열두 판이 ㄴ자의 오목 정점을 <b>우연히 피하고</b> 있었다.
                if (pn == "ㄴ자")
                {
                    double tCc = GradingGeometry.ParamAt(ruler, rc, 40.0 - 8.5, 30.0 - 8.5);
                    spots.Add(("오목 굽이를 품음", Wr112(tCc - 18.0), Wr112(tCc + 18.0)));
                }
            }
            ...
            foreach (var sp in spots)
            {
                ...
                // ★★★[검토 0911 · 높음7 후속] <b>오목 굽이를 품는 구간은 거절이 정답이다.</b>
                //   <para>좁은 노치(ㄴ자의 40×30 자리)의 <b>양쪽 벽 자리가 서로 겹친다</b> —
                //   두 변의 바깥 법선이 <b>둘 다 노치 안</b>을 향하므로(실측: 정방향·역방향
                //   <b>모두 자기교차 1곳</b>) 하나의 단순 폴리곤으로 표현할 수가 없다.
                //   그러니 "만들어진다"가 아니라 <b>"까닭을 말하고 거절한다"</b>가 통과 조건이다.</para>
                //   <para>★<b>겹치는 까닭은 기하에서 따라 나온 추론</b>이고 겹침 면적을 직접
                //   재 본 것은 아니다 — 재려면 두 쪽 벽 footprint를 각각 만들어 교집합을 재야 한다.</para>
                if (sp.Name == "오목 굽이를 품음")
                {
                    bool said = !made && (why.Contains("자기교차") || why.Contains("고리"));
                    Check($"{tag} ★★★거절하고 <b>까닭을 말한다</b>(좁은 노치에서 벽 자리가 겹친다)",
                          said, made ? $"만들어졌다({poly!.Count}점) — 겹치는데 폴리곤이 나오면 확인해야 한다"
                                     : $"까닭 '{why}'");
                    if (said) okAll++; else badAll++;
                    continue;
                }
                ...
                // ③★★★[검토 0911 · 높음7] 안쪽 변을 <b>되돌리면 찍은 선이 나오나.</b>
                //   <para>종전엔 "찍은 선까지 거리가 어디서나 <c>run</c>"으로 쟀다. 그런데
                //   <b>오목한 굽이</b>에서는 그 자가 틀린 것을 가리킨다 — 마이터 점은 두
                //   <b>변</b>에서 정확히 1.05m인데 <b>꺾인 폴리선</b>까지는 1.05√2 = 1.485m다
                //   (최근접점이 변의 수직발이 아니라 <b>꼭짓점 자신</b>이므로).
                //   검토가 그 0.435m를 결함으로 보고했는데, 정지 엔진이 폴리곤을
                //   <b>버퍼로 바깥으로 밀어</b> 단을 쌓으므로 <b>되돌려서 찍은 선이 나오면</b>
                //   그것이 맞는 점이다. 그래서 자를 바꾼다 — <b>되돌리기</b>로 잰다.</para>
                double inLo = double.MaxValue, inHi = 0;
                {
                    var innerEdge = new List<Point3>();
                    for (int k = 0; k < parts.Inner && k < poly.Count; k++) innerEdge.Add(poly[k]);
                    List<Point3>? back = null;
                    foreach (int sg in new[] { +1, -1 })
                    {
                        var cand = GradingGeometry.OffsetOpen(innerEdge, run112, sg, picked[0].Z);
                        if (cand == null || cand.Count < 2) continue;
                        double d0 = System.Math.Sqrt((cand[0].X-picked[0].X)*(cand[0].X-picked[0].X)
                                                   + (cand[0].Y-picked[0].Y)*(cand[0].Y-picked[0].Y));
                        if (d0 < 1.0) { back = cand; break; }
                    }
                    if (back == null) { inLo = 0; inHi = 99; }
                    else
                        foreach (var q in back)
                        {
                            double best = double.MaxValue;
                ...
                // ⑥★★★[검토 0911] <b>부지를 품지 않는다</b> — 검토가 잰 22판에서 정상은 0,
                //   틀린 후보는 전부(4/4·6/6·8/8)로 <b>중간값이 하나도 없었다</b>.
                //   (검토가 처음 제안한 "면적 상한"은 정상 판 12개 중 10개가 헛울려 안 쓴다.)
                int eats = 0;
                foreach (var q in pad) if (In112(poly, q.X, q.Y)) eats++;
                bool all = gap < 3.0 && cross == 0 && inHi < 0.3
                        && outer != null && wOff < 1.0
                        && zLo >= bLo && zHi <= bHi && eats == 0;
                Check($"{tag} ★★★일곱 불변식", all,
                      $"닫힘 {gap:0.##}m · 교차 {cross} · 되돌리기 오차 {inLo:0.###}~{inHi:0.###}m(0이어야)"
                    + $" · 날개↔링 {wOff:0.##}m · Z[{zLo:0.#}..{zHi:0.#}] 띠[{bLo:0.#}..{bHi:0.#}]"
                    + $" · 부지 품음 {eats}/{pad.Count}");
                if (all) okAll++; else badAll++;
            }
        }
    }
    Check("S112 ★★★모든 판이 통과한다", badAll == 0 && okAll >= 14,
          $"통과 {okAll} · 실패 {badAll} (14판이어야 — 판 둘 × 방향 둘 × 자리 셋 + ㄴ자 오목 판 둘)");
}
    ...
    if (ruler114 != null)
    {
        ...
        if (vpoly114 != null)
        {
            ...
            foreach (bool noPlat in new[] { false, true })
            {
                ...
                int ringN = v.Rings.Count, ptN = 0, inPad = 0, above = 0, below = 0;
                double zLo = double.MaxValue, zHi = double.MinValue;
                ...
                foreach (var r in v.Rings)
                    foreach (var q in r)
                    {
                        ...
                        if (!InPad(pad114, q.X, q.Y)) continue;
                        inPad++;
                        // ★★★[검토 0911] <b>절토 판에서는 깎지 않고 "묻는다".</b>
                        //   부지 안 점의 Z가 계획고(100)보다 <b>위</b>다 — <c>PasteSurface</c>가
                        //   덮어쓰므로 부지가 흙더미에 <b>묻힌다</b>. 깎이는 것은 성토 판이다.
                        //   화면에서 찾을 모양이 정반대이므로 이것을 갈라 센다.
                        if (q.Z > 100.0) above++; else below++;
                        double t = GradingGeometry.ParamAt(pad114, c114, q.X, q.Y);
                        var b = GradingGeometry.PointAtParam(pad114, c114, t);
                        deepest = System.Math.Max(deepest,
                            System.Math.Sqrt((q.X-b.X)*(q.X-b.X) + (q.Y-b.Y)*(q.Y-b.Y)));
                    }
                inPadBy[noPlat ? 1 : 0] = inPad;
                string lbl = noPlat ? "NoPlatform 켬" : "NoPlatform 끔";
                Console.WriteLine($"      S114 [{lbl}] 부지 안 점의 표고 — 계획고(100) <b>위 {above}개</b>"
                    + $" · 아래 {below}개 (절토 판이라 묻는다 · 깎이는 것은 성토 판)");
                ...
                // ★★★[검토 0911] <b>"&gt; 0"으로 단언하면 안 된다</b> — 1196이 11960이 되어도 통과한다.
                //   골든값 ± 허용오차로 못 박고, <b>위/아래</b>도 함께 단언한다.
                Check($"S114 ★★★[{lbl}] <b>걸림돌 기록</b> — 부지 안 침범이 <b>잰 그 크기</b>다"
                      + "(속이 부지 밖 쐐기라 안쪽 변에서 부지 쪽으로 자란다)",
                      System.Math.Abs(inPad - 1196) <= 60 && System.Math.Abs(deepest - 29.8) < 1.0
                      && above == inPad && below == 0,
                      $"부지 안 점 <b>{inPad}개</b>(잰 값 1196 ±60) · 가장 깊이 {deepest:0.#}m(잰 값 29.8)"
                    + $" · 위 {above}/아래 {below}(절토 판이라 전부 위여야) · 링 {ringN}개"
                    + $" · Z[{(ptN > 0 ? zLo : 0):0.#}..{(ptN > 0 ? zHi : 0):0.#}]");
            }
            // ★★★<b>이것이 영구 불변식이다</b> — 고쳐도 참이어야 한다.
            //   <c>NoPlatform</c>은 <b>안쪽 평지 링 하나</b>만 막으므로 계단은 그대로 쌓인다.
            //   그래서 부지 안 점 수가 <b>켜도 끄도 같다</b>. "고치면 거짓이 되는" 자가 아니라
            //   "<c>NoPlatform</c>이 이 문제를 못 고친다"는 사실을 못 박는 자다.
            Check("S114 ★★★<c>NoPlatform</c>은 부지 안 침범을 <b>못 고친다</b>(켜도 끄도 같다)",
                  inPadBy[0] == inPadBy[1] && inPadBy[0] > 0,
                  $"끔 {inPadBy[0]}개 · 켬 {inPadBy[1]}개 — 같아야 한다"
                + "(안쪽 평지 링 하나만 막고 계단은 그대로 쌓이므로)");
        }
    }
}
    ...
    // ══ ① ㄷ자 부지의 오목한 안쪽 벽 ══
    {
        ...
        if (rulerC != null && oldC != null)
        {
            ...
            foreach (double w in new[] { 4.0, 8.0, 16.0, 40.0 })
            {
                ...
                // ★★[검토 0911] <b>면적 상한은 단언에 쓰지 않는다.</b> 검토가 처음 제안했지만
                //   다시 재 보니 <b>정상 판 12개 중 10개가 헛울린다</b>(직사각 변가운데 5,290 vs 상한 4,964).
                //   대신 <c>부지를 품나</c>를 쓴다 — 22판 실측에서 정상은 <b>0</b>,
                //   틀린 후보는 <b>전부</b>(4/4·6/6·8/8)로 중간값이 하나도 없었다.
                bool good = eats == 0;
...
// ══ S116 ★★★[검토 0911 · 높음4·5·6] 코너 판정 · 날개 방향 · 마감 링 ══════════════════
//
//   <para><b>높음4 — 유효 창이 문턱이 아니라 <u>내밀기 거리</u>로 정해졌다.</b>
//   종전엔 자 위 점을 <b>계획 경계에 투영</b>했다. 볼록 코너에서 자의 코너 다리 전체가
//   계획 정점 하나로 투영되어 고원이 생긴다. 검토 실측:</para>
//   <code>
...
//   </code>
//
//   <para>★<b>먼저 지은 판(고리 꺾임을 ±3m 창으로 합치기)은 재서 버렸다.</b>
//   검토 실측으로 마이터 3곳/3곳 · 라운드 4곳/<b>0곳</b> — 라운드·내밀기 40m에서
//   코너 호가 62.8m이고 한 마디가 5.24m라 ±3m 창에 정점이 하나만 들어가 7.5° &lt; 30°였다.
//   지금 방식은 <b>계획 정점을 자에 투영</b>한다(방향만 뒤집었다).</para>
//
//   <para><b>높음5 — 같은 셈이 세 군데 있었고 하나만 고쳐져 있었다.</b>
//   <c>WingLine</c>이 옛 식(구간 <b>바깥쪽</b> 이웃)을 써서 화면의 날개와 폴리곤의 날개가
//   <b>코너마다 90° 달랐다</b>. <c>DH-날개벽선</c>은 JACK이 "보고 결정"하려고 그리는 미리보기다 —
//   지어질 것과 다른 규칙을 쓰면 그것은 미리보기가 아니라 <b>거짓</b>이다.</para>
{
    Console.WriteLine("\n== S116 코너 판정 · 날개 방향 · 마감 링 ==");
    ...
    var nook116 = new List<Point3> { new(0,0,100), new(80,0,100), new(80,30,100),
                                     new(40,30,100), new(40,60,100), new(0,60,100) };
    GradingParams Par116(bool mitre) => new GradingParams
    {
        CutBenchHeight = 5, FillBenchHeight = 5, CutBenchWidth = 1, FillBenchWidth = 1,
        CutSlope = 1.5, FillSlope = 1.5, CellSize = 0.5, MaxBenches = 20, MaxRise = 60,
        VertexSpacing = 2.0, MinSlope = 0.01, MinFaceRun = 0.005, MiterConvex = mitre, MiterLimit = 2.0,
    };

    // ══ ① 창이 <b>내밀기 거리·조인 방식</b>에 안 끌려가나 ══
    //    ★검토가 재서 버린 판(고리 꺾임 창)은 라운드·40m에서 0곳이었다 — 그 판까지 넣어 잰다.
    {
        int bad = 0; string worst = "";
        foreach (bool mitre in new[] { true, false })
            foreach (double off in new[] { 1.05, 8.5, 20.0, 40.0, 60.0 })
            {
                var par = Par116(mitre);
                var ring = GradingGeometry.OffsetRingForTest(pad116, off, par);
                if (ring == null) { bad++; continue; }
                var rc = GradingGeometry.CumLen2D(ring);
                double tot = rc[^1];
                var corners = GradingGeometry.CornerParamsOf(pad116, ring, rc);
                int hitN = 0, all = 0, run = 0, maxRun = 0;
                for (double t = 0; t < tot; t += 0.5)
                {
                    all++;
                    if (GradingGeometry.AtRingCorner(corners, t, tot, 1.5))
                    { hitN++; run++; maxRun = System.Math.Max(maxRun, run); }
                    else run = 0;
                }
                double frac = all == 0 ? 0 : 100.0 * hitN / all;
                double win = maxRun * 0.5;
                bool ok = corners.Count == 4 && win <= 4.0 && frac <= 12.0;
                Console.WriteLine($"      S116 ① {(mitre ? "마이터" : "라운드")}·내밀기 {off,5:0.00}m"
                    + $" — 코너 {corners.Count}곳 · 호 {frac,4:0.0}% · 창 {win,5:0.0}m {(ok ? "✓" : "<b>⚠</b>")}");
                if (!ok) { bad++; if (worst.Length == 0) worst = $"{(mitre ? "마이터" : "라운드")}·{off:0.##}m — 코너 {corners.Count}곳 · 창 {win:0.0}m"; }
            }
        Check("S116 ★★★①코너 창이 <b>내밀기 거리와 조인 방식에 안 끌려간다</b>(열 판)",
              bad == 0,
              bad == 0 ? "마이터·라운드 × 내밀기 다섯 = 열 판 전부 코너 4곳 · 창 3m"
                       : $"<b>{bad}판 실패</b> — {worst} (버린 판: 라운드·40m에서 0곳)");
    }

    // ══ ② 코너 자리가 <b>계획 꼭짓점</b>에 앉나 ══
    {
        foreach (bool mitre in new[] { true, false })
        {
            var par = Par116(mitre);
            var ring = GradingGeometry.OffsetRingForTest(pad116, 8.5, par);
            if (ring == null) continue;
            var rc = GradingGeometry.CumLen2D(ring);
            var corners = GradingGeometry.CornerParamsOf(pad116, ring, rc);
            // 자 위 그 자리를 계획 경계에 되투영하면 계획 꼭짓점이어야 한다
            double worst = 0;
            var cB = GradingGeometry.CumLen2D(pad116);
            foreach (double t in corners)
            {
                var q = GradingGeometry.PointAtParam(ring, rc, t);
                double tp = GradingGeometry.ParamAt(pad116, cB, q.X, q.Y);
                var b2 = GradingGeometry.PointAtParam(pad116, cB, tp);
                double best = double.MaxValue;
                foreach (var v in pad116)
                    best = System.Math.Min(best, System.Math.Sqrt((b2.X-v.X)*(b2.X-v.X) + (b2.Y-v.Y)*(b2.Y-v.Y)));
                worst = System.Math.Max(worst, best);
            }
            Console.WriteLine($"      S116 ② {(mitre ? "마이터" : "라운드")} 8.5m 링 — 코너 {corners.Count}곳"
                + $" · 계획 꼭짓점과 가장 먼 것 {worst:0.###}m");
            Check($"S116 ★★★②[{(mitre ? "마이터" : "라운드")}] 코너 자리가 <b>계획 꼭짓점</b>에 앉는다",
                  corners.Count == 4 && worst < 0.5,
                  $"코너 {corners.Count}곳(4이어야) · 계획 꼭짓점과 가장 먼 것 {worst:0.###}m");
        }
    }

    // ══ ③ 완만한 꺾임은 코너로 안 본다 — 문턱이 <b>계획 폴리곤</b>에 걸린다 ══
    {
        // 한 꼭짓점만 완만하게(약 27.6°) 꺾인 판
        var gentle = new List<Point3> { new(0,0,100), new(80,0,100), new(120,21,100), new(120,80,100), new(0,80,100) };
        var par = Par116(true);
        var ring = GradingGeometry.OffsetRingForTest(gentle, 8.5, par);
        if (ring != null)
        {
            var rc = GradingGeometry.CumLen2D(ring);
            var c30 = GradingGeometry.CornerParamsOf(gentle, ring, rc, 30.0);
            var c10 = GradingGeometry.CornerParamsOf(gentle, ring, rc, 10.0);
            Console.WriteLine($"      S116 ③ 완만한 꼭짓점 하나 — 문턱 30°: {c30.Count}곳 · 문턱 10°: {c10.Count}곳");
            Check("S116 ★★★③문턱이 <b>계획 폴리곤의 꺾임</b>에 걸린다(완만한 꼭짓점을 거른다)",
                  c30.Count == c10.Count - 1 && c10.Count == 5,
                  $"30°에서 {c30.Count}곳 · 10°에서 {c10.Count}곳 (완만한 것 하나가 걸러져야)");
        }
    }
    ...
    // ══ ④ 화면의 날개와 폴리곤의 날개가 <b>같은 방향</b>인가 ══
    {
        ...
        {
            var par = Par116(true);
            var ring = GradingGeometry.OffsetRingForTest(pad, 8.5, par);
            var day = GradingGeometry.OffsetRingForTest(pad, 60.0, par);
            if (ring == null || day == null) continue;
            for (int i = 0; i < ring.Count; i++) ring[i] = new Point3(ring[i].X, ring[i].Y, 105.0);
            var rc = GradingGeometry.CumLen2D(ring);
            double tot = rc[^1];
            var corners = GradingGeometry.CornerParamsOf(pad, ring, rc);
            int checkedN = 0, diff = 0, atCorner = 0; double worstDeg = 0;
            ...
            Console.WriteLine($"      S116 ④ [{nm}] 잰 자리 {checkedN}개(그중 코너 {atCorner}개)"
                + $" · 방향이 다른 곳 {diff}개 · 최악 {worstDeg:0.##}°");
            // ★★코너 자리를 <b>한 번도 안 밟으면</b> 이 검사는 결함을 못 본다(옛 S108이 그랬다).
            Check($"S116 ★★★④[{nm}] <b>화면의 날개와 폴리곤의 날개가 같은 방향</b>이다",
                  checkedN > 0 && atCorner > 0 && diff == 0,
                  $"잰 자리 {checkedN}개(코너 {atCorner}개 — 0이면 결함을 못 본다) · 다른 곳 {diff}개"
                + $" · 최악 {worstDeg:0.##}° (고치기 전: 코너마다 90° 달랐다)");
        }
    }

    // ══ ⑤ [높음6] 옛 데이라잇이 <b>꼬여</b> 있어도 마감 링을 살리나 ══
    {
        var par = Par116(true);
        var good = GradingGeometry.OffsetRingForTest(pad116, 60.0, par);
        // 나비넥타이 — 두 점을 맞바꿔 자기교차를 만든다
        var bow = good == null ? null : new List<Point3>(good);
        if (bow != null && bow.Count > 20)
        { var t0 = bow[5]; bow[5] = bow[bow.Count/2]; bow[bow.Count/2] = t0; }
        foreach (var (nm, old) in new[] { ("멀쩡한 링", good), ("꼬인 나비넥타이", bow) })
        {
            var outer = GradingGeometry.OuterDaylightRing(pad116, old, 15.85, par);
            double far = 0;
            var cB = GradingGeometry.CumLen2D(pad116);
            if (outer != null)
                foreach (var q in outer)
                {
                    double tp = GradingGeometry.ParamAt(pad116, cB, q.X, q.Y);
                    var b2 = GradingGeometry.PointAtParam(pad116, cB, tp);
                    far = System.Math.Max(far, System.Math.Sqrt((q.X-b2.X)*(q.X-b2.X) + (q.Y-b2.Y)*(q.Y-b2.Y)));
                }
            Console.WriteLine($"      S116 ⑤ [{nm}] 마감 링 {outer?.Count ?? 0}점 · 경계에서 최대 {far:0.##}m"
                + $"\n        {GradingGeometry.LastOuterRingLog}");
            Check($"S116 ★★★⑤[{nm}] 마감 링이 <b>옛 데이라잇까지 나간다</b>(꼬였어도 안 버린다)",
                  outer != null && far > 80.0,
                  $"경계에서 최대 {far:0.##}m (84.85m 근처여야 · 옛 데이라잇을 버리면 22.42m가 된다)");
        }
    }

    // ══ ⑥ [높음·새] TIN이 <b>부지 주변만</b> 덮어도 폴리곤이 나오나 ══
    //    ★현장 측량은 보통 부지 주변만 덮는다. 마감 링은 60m까지 나가므로
    //      "못 물으면 거절"로 두면 <b>현장 대부분에서 기능이 멈춘다</b>.
    {
        var par = Par116(true);
        var ring = GradingGeometry.OffsetRingForTest(pad116, 8.5, par);
        var oldDay = GradingGeometry.OffsetRingForTest(pad116, 60.0, par);
        if (ring != null && oldDay != null)
        {
            for (int i = 0; i < ring.Count; i++) ring[i] = new Point3(ring[i].X, ring[i].Y, 105.0);
            // ★옛 데이라잇 선의 Z = 그 자리 원지반 표고(정의상 사면이 원지반과 닿는 선)
            for (int i = 0; i < oldDay.Count; i++) oldDay[i] = new Point3(oldDay[i].X, oldDay[i].Y, 140.0);
            var rc = GradingGeometry.CumLen2D(ring);
            double rt = rc[^1];
            foreach (var (nm, gb) in new (string, IGroundSurface)[]
            {
            ...
            {
                var poly = GradingGeometry.BuildWallPolygon(
                    ring, rc, rt * 0.08, rt * 0.22, pad116, GradingGeometry.CumLen2D(pad116),
                    oldDay, gb, par, up: true, wallSlope: par.MinSlope, cornerTol: 1.5, out string why6);
                var pp = GradingGeometry.LastWallPolyParts;
                double zLo = double.MaxValue, zHi = double.MinValue;
                if (poly != null) foreach (var q in poly) { zLo = System.Math.Min(zLo, q.Z); zHi = System.Math.Max(zHi, q.Z); }
                Console.WriteLine($"      S116 ⑥ [{nm}] {(poly == null ? "거절 — " + why6 : $"{poly.Count}점 · Z[{zLo:0.#}..{zHi:0.#}] · 바깥 변 {pp.Far}점")}");
                Check($"S116 ★★★⑥[{nm}] 폴리곤이 <b>나온다</b>(TIN 범위가 좁아도 멈추지 않는다)",
...
//     지금은 <b>폴리곤이 도는 방향</b>으로 정한다 — 모양을 안 가린다.
{
    // 점이 폴리곤 안인가(홀짝 규칙) — 이 검사만을 위한 자다.
    static bool Inside(List<Point3> poly, double x, double y)
    {
        bool inside = false;
        int n = poly.Count;
        for (int i = 0, j = n - 1; i < n; j = i++)
    ...
    // L자 부지 — 오목 굽이가 하나(40×40에서 오른쪽 위 20×20을 파낸 모양).
    var L = new List<Point3>
    {
        new(0,0,0), new(40,0,0), new(40,20,0), new(20,20,0), new(20,40,0), new(0,40,0),
    };
    // ㄷ자 부지 — 오목 굽이가 둘.
    var U = new List<Point3>
    {
        new(0,0,0), new(40,0,0), new(40,40,0), new(30,40,0), new(30,15,0),
        new(10,15,0), new(10,40,0), new(0,40,0),
    };
    var box = new List<Point3> { new(0,0,0), new(40,0,0), new(40,40,0), new(0,40,0) };

    foreach (var (name, ring) in new[] { ("사각형", box), ("L자", L), ("ㄷ자", U) })
    {
        var cum = GradingGeometry.CumLen2D(ring);
        double tot = cum[cum.Length - 1];
        int bad = 0, n = 0;
        double worstT = -1;
        // 둘레를 0.5m마다 훑어 <b>모든 자리</b>에서 바깥인지 본다(꼭짓점 언저리는 건너뛴다 —
        // 정확히 모서리 위는 방향이 정의되지 않는다).
        for (double t = 0.25; t < tot; t += 0.5)
        {
            bool nearCorner = false;
            for (int i = 0; i < cum.Length - 1; i++)
                if (System.Math.Abs(t - cum[i]) < 0.6) { nearCorner = true; break; }
            if (nearCorner) continue;
            var o = GradingGeometry.OutwardAt(ring, cum, t, 0.5);
            n++;
            if (Inside(ring, o.X, o.Y)) { bad++; if (worstT < 0) worstT = t; }
        }
        Check($"S105 ★{name}: 모든 자리에서 바깥을 가리킨다", bad == 0 && n > 0,
              bad == 0 ? $"{n}자리 확인" : $"{bad}/{n}자리 안쪽을 가리킴(첫 자리 {worstT:0.#}m)");
    }

    // ★시계·반시계 어느 쪽으로 그렸든 같아야 한다 — 사용자가 그린 방향을 우리가 못 정한다.
    var cw = new List<Point3>(box); cw.Reverse();
    var cumCw = GradingGeometry.CumLen2D(cw);
    var oCw = GradingGeometry.OutwardAt(cw, cumCw, 20, 1.0);
    Check("S105 ★★시계 방향으로 그린 부지도 바깥을 가리킨다",
          !Inside(cw, oCw.X, oCw.Y), $"({oCw.X:0.#},{oCw.Y:0.#})");
}
...
// ══ S107 ★★★[JACK 0910 · 검토 0910] <b>TIN을 실제로 떠서 잰다</b> ══════════════
//
//   ★<b>왜 이 검사가 이제야 생겼나.</b> 우리는 여태 <b>링</b>만 재면서 <b>TIN</b>이 깨진다고 말하고 있었다.
//     현장 로그가 잰 것은 <i>"링 최장변 14.67m인데 완성 지표면 TIN 최장변 55.71m"</i> —
//     <b>링은 이미 괜찮았고 TIN이 깨졌다</b>. 그런데 링을 89m에서 1m로 만드는 수정을 하고
//     "고쳤다"고 했다(0910, 되돌림). <b>증상과 따로 노는 값을 재고 있었기 때문</b>이다.
//
//   ★<b>이제 잴 수 있다.</b> 두 가지가 맞물렸다:
//     ①TIN에 <b>무엇이 들어가는지</b> 정하는 규칙을 Core로 옮겼다(<see cref="BreaklinePrep.SplitRingRuns"/>) —
//       그래서 이 검사가 <b>출하되는 그 규칙</b>을 먹는다(「검사 입력은 출하 입력이어야 한다」).
//     ②삼각망은 NTS가 이미 들고 있다(<c>DelaunayTriangulationBuilder</c>, Core의 의존성).
//
//   ★<b>정직하게 단서를 단다.</b> NTS는 Civil이 아니고, 여기 값은 브레이크라인을 <b>강제하지 않은</b>
//     삼각화다. 그러니 이 숫자로 "Civil에서 부채꼴이 사라진다"를 단정하지 않는다.
//     이 자가 말해 주는 것은 <b>지시선이 없는 빈 면이 얼마나 넓게 남았나</b>이고,
//     부채꼴은 바로 그 빈 면을 삼각화가 길쭉하게 건너서 생긴다.
{
    Console.WriteLine("\n== S107 TIN 실측 — 지시선 없는 빈 면이 얼마나 남나 ==");

    var pad107 = new List<Point3> { new(0,0,100), new(80,0,100), new(80,60,100), new(0,60,100) };
    var cum107 = GradingGeometry.CumLen2D(pad107);
    double per107 = cum107[^1];
    var gnd107 = new FlatGround(140);       // 절토 40m
    var par107 = new GradingParams
    {
        CutBenchHeight = 5, FillBenchHeight = 5, CutBenchWidth = 1, FillBenchWidth = 1,
        CutSlope = 1.5, FillSlope = 1.5, CellSize = 0.5, MaxBenches = 20, MaxRise = 60,
        VertexSpacing = 2.0, MinSlope = 0.01, MinFaceRun = 0.005, MiterConvex = true, MiterLimit = 2.0,
        ...
        TinInput(VirtualSlope v)
    {
        ...
        return (pts, runN, dives, dmax, lost);
    }
        ...
        TinEdgesCore(List<NetTopologySuite.Geometries.Coordinate> pts, Func<double, double, bool> keep)
    {
        ...
        var edges = b.GetEdges(gf);
        double mx = 0, ax = 0, ay = 0; int o5 = 0, o10 = 0, n = 0;
        for (int i = 0; i < edges.NumGeometries; i++)
        {
            ...
            if (d > 5.0) o5++;
            if (d > 10.0) o10++;
            if (d > mx) { mx = d; ax = c0.X; ay = c0.Y; }
        }
        return (mx, o5, o10, n, ax, ay);
    }
    ...
    Check("S107 ★재현: 부분 구간이면 링이 끊겨 브레이크라인이 갈라진다",
          tiW.Dives > 0 && tiW.Runs > tiP.Runs,
          $"끊긴자리 {tiP.Dives} → {tiW.Dives}곳 · 브레이크라인 {tiP.Runs} → {tiW.Runs}줄"
        + $" (최장 다이브 {tiW.DiveMax:0.0}m)");

    // ⑤ ★<b>점이 사라져서 생긴 구멍은 아니다</b> — 후보 하나를 못박아 배제한다.
    //   끊긴 조각에서 점이 증발했다면 그것이 원인일 텐데, 실측은 점이 <b>더 많다</b>고 말한다.
    // ★★[0910 실측 정정] 처음엔 <c>TIN 점 수가 안 줄었다</c>로 썼는데 <b>떨어졌다</b>(17612 → 12950).
...
// ── S128 ★★★[JACK 0916 스샷 <i>"깨지고 난리났어"</i>] <b>틈이 생기면 잡는다</b> ──
//
//   <para><b>현장 계측(12:51 로그)</b>: 발자국 307.8㎡ · 벽 덩이 248.3㎡ ·
//   「발자국 − 벽」 = 87.8㎡. 307.8 − 248.3 = 59.5여야 하는데 87.8이 나왔다.
//   차이 <b>28.3㎡</b>가 <b>벽도 뚜껑도 못 받는 빈 자리</b>다 —
//   Civil이 그 위를 긴 삼각형으로 건너질러 메우고, 되읽기에 «합성면 117.7 · 원래 면 120.00»이 찍혔다.</para>
//
//   <para>까닭은 둘 중 하나다(둘 다 같은 증상을 낸다):
//   ①덩이의 테두리가 <b>발자국 바깥으로 부푼다</b>(가운데 줄들을 곧은 선으로 건너뛰므로),
//   ②테두리가 <b>제 몸을 지른다</b>(자기교차) — 그러면 신발끈 넓이와 NTS가 쓰는 넓이가 어긋난다.</para>
//
//   <para>고침은 하나로 둘 다 잡는다 — <b>덩이를 발자국 안으로 먼저 자른다</b>
//   (<c>RingIntersect</c>는 결과가 항상 <b>성한 폴리곤</b>이다).
//   이 검사는 <b>출하되는 그 함수들</b>에 그대로 먹인다.</para>
{
    Console.WriteLine("\n== S128 벽 몫과 뚜껑 몫 사이에 틈이 없는가 ==");
    static double Ar(IReadOnlyList<Point3> r) => Math.Abs(Shoelace(r));
    static List<Point3> Box(double x0, double y0, double x1, double y1)
        => new List<Point3> { new Point3(x0, y0, 0), new Point3(x1, y0, 0),
                              new Point3(x1, y1, 0), new Point3(x0, y1, 0) };
    static double NetAr(List<(List<Point3> Ring, List<List<Point3>> Holes)> qs)
    { double a = 0; foreach (var q in qs) { a += Ar(q.Ring); foreach (var h in q.Holes) a -= Ar(h); } return a; }

    var foot = Box(0, 0, 40, 40);          // 발자국 1600㎡
    var wedge = Box(0, 0, 40, 100);        // 쐐기 4000㎡

    // ① <b>발자국 밖으로 부푼 덩이</b> — y가 −10까지 삐져나간다(밖 400㎡)
    {
        var lobe = Box(5, -10, 35, 30);    // 30×40 = 1200㎡ · 그중 y<0인 300㎡가 발자국 밖
        var Pw = new List<(List<Point3>, List<List<Point3>>)> { (lobe, new List<List<Point3>>()) };
        var flat = new List<IReadOnlyList<Point3>?> { lobe };

        // (가) 고치기 전 — 덩이를 그대로 쓴다
        var cut0 = GradingGeometry.RingSubtract(foot, flat, 0, out string cl0, 1.0);
        double gap0 = Ar(foot) - Ar(lobe) - NetAr(cut0);
        Console.WriteLine($"      S128 [부푼 덩이 · 고치기 전] 발자국 {Ar(foot):F1} − 덩이 {Ar(lobe):F1}"
                        + $" − 뚫을 몫 {NetAr(cut0):F1} = <b>{gap0:F1}㎡</b>");
        Check("S128 ★이 검사가 <b>뜻이 있다</b> — 고치기 전엔 장부가 어긋난다", Math.Abs(gap0) > 1.0,
              $"어긋남 {gap0:F1}㎡");

        // (나) 고친 뒤 — 발자국 안으로 먼저 자른다
        var fit = GradingGeometry.RingIntersect(Pw, foot, 0, out string fl, 1.0);
        var flat2 = new List<IReadOnlyList<Point3>?>(); foreach (var q in fit) flat2.Add(q.Ring);
        var cut1 = GradingGeometry.RingSubtract(foot, flat2, 0, out string cl1, 1.0);
        var lid1 = GradingGeometry.RingSubtract(wedge, flat2, 0, out string ll1, 1.0);
        double aFit = NetAr(fit), gap1 = Ar(foot) - aFit - NetAr(cut1);
        double gapW = Ar(wedge) - aFit - NetAr(lid1);
        Console.WriteLine($"      S128 [부푼 덩이 · 고친 뒤] 맞춘 덩이 {aFit:F1}㎡({fl})");
        Console.WriteLine($"      S128   발자국 {Ar(foot):F1} − 벽 {aFit:F1} − 뚫을 몫 {NetAr(cut1):F1} = <b>{gap1:F2}㎡</b>");
        Console.WriteLine($"      S128   쐐기 {Ar(wedge):F1} − 벽 {aFit:F1} − 뚜껑 {NetAr(lid1):F1} = <b>{gapW:F2}㎡</b>");
        Check("S128 ★★★<b>발자국 = 벽 + 뚫을 몫</b>(틈 없음)", Math.Abs(gap1) < 1.0, $"{gap1:F2}㎡");
        Check("S128 ★★★<b>쐐기 = 벽 + 뚜껑</b>(틈 없음)", Math.Abs(gapW) < 1.0, $"{gapW:F2}㎡");
        Check("S128 ★맞춘 덩이는 <b>발자국 안</b>에만 있다", Math.Abs(aFit - 900.0) < 5.0, $"{aFit:F1}㎡(기대 900)");
    }

    // ② <b>제 몸을 지르는 테두리</b>(자기교차) — 신발끈 넓이와 NTS 넓이가 어긋난다
    {
        // 나비 모양: 두 삼각형이 가운데서 만난다. 신발끈은 서로 상쇄돼 <b>0에 가깝게</b> 나온다.
        var bow = new List<Point3> { new Point3(5, 5, 0), new Point3(35, 35, 0),
                                     new Point3(5, 35, 0), new Point3(35, 5, 0) };
        var Pw = new List<(List<Point3>, List<List<Point3>>)> { (bow, new List<List<Point3>>()) };
        var fit = GradingGeometry.RingIntersect(Pw, foot, 0, out string fl2, 1.0);
        var flat3 = new List<IReadOnlyList<Point3>?>(); foreach (var q in fit) flat3.Add(q.Ring);
        var cut2 = GradingGeometry.RingSubtract(foot, flat3, 0, out string cl2, 1.0);
        double aFit2 = NetAr(fit), gap2 = Ar(foot) - aFit2 - NetAr(cut2);
        Console.WriteLine($"      S128 [나비 테두리] 신발끈 {Ar(bow):F1}㎡ → 맞춘 뒤 {aFit2:F1}㎡({fl2})");
        Console.WriteLine($"      S128   발자국 {Ar(foot):F1} − 벽 {aFit2:F1} − 뚫을 몫 {NetAr(cut2):F1} = <b>{gap2:F2}㎡</b>");
        Check("S128 ★자기교차 테두리도 <b>성하게</b> 바뀐다(넓이 > 0)", aFit2 > 100, $"{aFit2:F1}㎡");
        Check("S128 ★★★자기교차여도 <b>틈이 없다</b>", Math.Abs(gap2) < 1.0, $"{gap2:F2}㎡");
    }
}

// ── S129 ★★★[검토 0916 · 치명1·2·4] <b>뚜껑 바깥 변을 딸 때 조용히 망가지지 않는가</b> ──
//
//   <para>검토가 오프라인 계측기로 <b>출하 함수 그대로</b> 돌려 잡아 준 것들이다:
//   ①못 찾은 자리를 <b>소리 없이 건너뛰어</b> 링이 <b>17.19m</b>를 곧은 선으로 가로질렀다(정상 칸 1.00m),
//   ②그런데도 «자가검증 최악 <b>0.000m</b>»이 찍혔다 — 찾은 점에서 |원지반−정지면|을 재니
//   <b>정의상 늘 0</b>이라 아무것도 검증하지 않았다,
//   ③25자리 중 <b>2곳만</b> 찾은 27점짜리 링이 「넓이 0.3~3배」 관문을 그대로 통과했다.</para>
//
//   <para>여기서 재는 것은 <b>출하되는 그 함수</b>(<c>WallTrim.DaylightOutward</c>)다.</para>
{
    Console.WriteLine("\n== S129 뚜껑 바깥 변(DaylightOutward) ==");
    // 자 = 반시계 정사각형. 아래 변(t=0..100)의 <b>바깥</b>은 −y 쪽이다(OutwardAt 규약).
    var ruler = new List<Point3> { new Point3(0, 0, 0), new Point3(100, 0, 0),
                                   new Point3(100, 100, 0), new Point3(0, 100, 0) };
    var cum = GradingGeometry.CumLen2D(ruler);
    Func<double, double, double?> g0 = (x, y) => 100.0;

    // ★0 <b>고른 데이라잇</b> — 아래로 10m 자리에서 정지면이 원지반을 만난다
    {
        Func<double, double, double?> d0 = (x, y) => 100.0 + (Math.Abs(y) - 10.0);
        var pts = WallTrim.DaylightOutward(ruler, cum, 5, 90, g0, d0, 60, 90, 0.5, 0,
                                           out int nMiss, out double mx, out int nMulti, out string lg);
        Console.WriteLine("      S129 [고른 데이라잇] " + lg);
        double worstY = 0; foreach (var q in pts) worstY = Math.Max(worstY, Math.Abs(q.Y + 10.0));
        Check("S129 ★자리 수가 <b>안 준다</b>(91자리 → 91점)", pts.Count == 91, $"{pts.Count}점");
        Check("S129 ★못 찾은 자리가 없다", nMiss == 0, $"못 찾음 {nMiss}");
        Check("S129 ★뿌리를 <b>두 번 세지 않는다</b>(표본이 뿌리에 딱 떨어져도)", nMulti == 0, $"뿌리 둘 이상 {nMulti}곳");
        Check("S129 ★★★<b>이론값 y = −10.00</b>과 맞는다", worstY < 0.01, $"최악 {worstY:F4}m");
        Check("S129 ★한 걸음이 칸(1.00m)과 같다", mx < 1.05, $"{mx:F3}m");
    }

    // ★0b ★★★<b>데이라잇 밖은 두 면이 같다</b> — 그 평평한 0에 속아 끝까지 달아나면 안 된다.
    //
    //   <para>이것이 <b>현장의 진짜 모양</b>이다. 합성면은 원지반을 깔고 시작하므로
    //   정지 구역 <b>밖에서는 정지면 = 원지반</b>이다(이 저장소가 스스로 적어 둔 성질).
    //   그래서 <c>diff</c>가 데이라잇 너머로 <b>쭉 0</b>이다.</para>
    //
    //   <para>검토 실측(참 데이라잇 10.00m): 「0도 부호 바뀜」으로 치면 찾을거리 80m에서
    //   <b>79.50m</b>를 딴다 — 그런데도 <b>단순·한걸음·못찾음이 다 통과</b>한다(고르게 밀려나 매끈하므로).
    //   <b>넓이 하나만으로는 못 막는다.</b></para>
    {
        // 안쪽(|y| < 10)은 정지면이 아래, 데이라잇(|y| = 10) <b>너머는 정확히 같다</b>.
        Func<double, double, double?> dFlat = (x, y) => Math.Abs(y) < 10.0 ? 100.0 - (10.0 - Math.Abs(y)) : 100.0;
        var pts = WallTrim.DaylightOutward(ruler, cum, 5, 90, g0, dFlat, 80, 90, 0.5, 0,
                                           out int nMiss, out double mx, out int nMulti, out string lg);
        Console.WriteLine("      S129 [밖은 두 면이 같음] " + lg);
        double worstY = 0, farY = 0;
        foreach (var q in pts) { worstY = Math.Max(worstY, Math.Abs(q.Y + 10.0)); farY = Math.Max(farY, -q.Y); }
        Console.WriteLine($"      S129   가장 먼 자리 y = −{farY:F2}(참값 −10.00 · 찾을거리 80m)");
        Check("S129 ★★★<b>평평한 0에 안 속는다</b>(참 데이라잇 y = −10.00)", pts.Count > 0 && worstY < 0.05,
              $"최악 {worstY:F3}m");
        Check("S129 ★★★<b>찾을거리 끝까지 달아나지 않는다</b>", farY < 12.0, $"가장 먼 y = −{farY:F2}");
    }

    // ★1 ★★★<b>가운데를 못 재도 링이 가로지르지 않는다</b>(검토 치명1 — 종전 17.19m)
    {
        Func<double, double, double?> gN = (x, y) => (x > 40 && x < 55) ? (double?)null : 100.0;
        Func<double, double, double?> d0 = (x, y) => 100.0 + (Math.Abs(y) - 10.0);
        var pts = WallTrim.DaylightOutward(ruler, cum, 5, 90, gN, d0, 60, 90, 0.5, 0,
                                           out int nMiss, out double mx, out int nMulti, out string lg);
        Console.WriteLine("      S129 [가운데 15m 조회 실패] " + lg);
        Check("S129 ★★★못 재도 <b>자리 수가 안 준다</b>", pts.Count == 91, $"{pts.Count}점");
        Check("S129 ★못 찾은 자리를 <b>세어서 알린다</b>", nMiss > 0, $"못 찾음 {nMiss}");
        Check("S129 ★★★<b>가로지르지 않는다</b>(한 걸음 ≤ 칸의 6배)", mx < 6.0, $"한 걸음 최악 {mx:F2}m");
    }

    // ★2 ★★★<b>뿌리가 둘이면 바깥 것을 고른다</b>(검토 높음4 — 종전 안쪽 3.000m를 골랐다)
    {
        //  (|y|−5)(|y|−15) — 5와 15에서 부호가 바뀐다. 뚜껑의 끝은 <b>바깥</b>인 15다.
        Func<double, double, double?> d2 = (x, y) => 100.0 + (Math.Abs(y) - 5.0) * (Math.Abs(y) - 15.0);
        var pts = WallTrim.DaylightOutward(ruler, cum, 5, 90, g0, d2, 60, 90, 0.5, 0,
                                           out int nMiss, out double mx, out int nMulti, out string lg);
        Console.WriteLine("      S129 [뿌리 둘] " + lg);
        double worstY = 0; foreach (var q in pts) worstY = Math.Max(worstY, Math.Abs(q.Y + 15.0));
        Check("S129 ★★★<b>가장 바깥 뿌리</b>(y = −15)를 고른다", pts.Count > 0 && worstY < 0.05,
              $"최악 {worstY:F4}m · 뿌리 둘 이상 {nMulti}곳");
        Check("S129 ★뿌리가 둘이던 자리를 <b>세어서 알린다</b>", nMulti > 50, $"{nMulti}곳");
        Check("S129 ★못 찾은 자리가 없다(뿌리 둘)", nMiss == 0, $"못 찾음 {nMiss}");
    }

    // ★3 <b>아무 데도 못 찾으면 빈손으로 돌려준다</b> — 가짜 점을 안 만든다
    {
        Func<double, double, double?> dHi = (x, y) => 200.0;   // 늘 정지면이 위 — 부호가 안 바뀐다
        var pts = WallTrim.DaylightOutward(ruler, cum, 5, 90, g0, dHi, 60, 90, 0.5, 0,
                                           out int nMiss, out double mx, out int nMulti, out string lg);
        Console.WriteLine("      S129 [부호가 안 바뀜] " + lg);
        Check("S129 ★★★못 찾으면 <b>빈손</b>(가짜 점을 안 만든다)", pts.Count == 0, $"{pts.Count}점");
    }

    // ★5 ★★★<b>자의 모서리</b>에 닿으면 — 바깥 방향이 90도 꺾여 엉뚱한 점이 나온다.
    //     고칠 수 없는 자리다(자 자체의 성질). 대신 <b>한 걸음 잣대가 그걸 잡아내야</b> 한다.
    {
        Func<double, double, double?> d0 = (x, y) => 100.0 + (Math.Abs(y) - 10.0);
        var pts = WallTrim.DaylightOutward(ruler, cum, 0, 100, g0, d0, 60, 100, 0.5, 0,
                                           out int nMiss, out double mx, out int nMulti, out string lg);
        Console.WriteLine("      S129 [모서리까지] " + lg);
        double nom = 100.0 / 100;
        Check("S129 ★★★모서리에서 망가진 걸 <b>한 걸음 잣대가 잡는다</b>(칸의 6배 넘음)",
              mx > nom * 6.0, $"한 걸음 최악 {mx:F2}m · 칸 {nom:F2}m");
    }

    // ★4 <b>제 몸을 지르는 링을 가려낸다</b>(검토 치명4 — 넓이는 모양의 잣대가 아니다)
    {
        var box = new List<Point3> { new Point3(0, 0, 0), new Point3(10, 0, 0),
                                     new Point3(10, 10, 0), new Point3(0, 10, 0) };
        var bow = new List<Point3> { new Point3(0, 0, 0), new Point3(10, 10, 0),
                                     new Point3(0, 10, 0), new Point3(10, 0, 0) };
        Check("S129 ★성한 링은 <b>성하다</b>고 한다", GradingGeometry.RingIsSimple(box), "네모");
        Check("S129 ★★★<b>나비 링을 가려낸다</b>", !GradingGeometry.RingIsSimple(bow), "나비");
        Console.WriteLine($"      S129 [모양 잣대] 네모 신발끈 {Math.Abs(Shoelace(box)):F1}㎡"
                        + $" · 나비 신발끈 {Math.Abs(Shoelace(bow)):F1}㎡ ← 넓이로는 못 가린다");
    }
}

// ── S130 ★★★[검토 0916 · 치명 · 현장 좌표로 재현] <b>덩이 테두리가 벽 없는 땅을 삼켰다</b> ──
//
//   <para><b>계측</b>(검토가 현장 좌표·판 그대로 재현): 덩이 링 <b>759.9㎡</b> vs 발자국 <b>404.9㎡</b> —
//   발자국보다 6배 깊고 바깥으로 <b>26.3m</b>. 덩이 ∖ 발자국 = <b>485.6㎡</b>.
//   옹벽 몫 합이 발자국 <b>전체의 2배</b>인데도 장부는 «맞음»이었다.
//   그리고 <b>아무것도 안 자르면 0.0㎡</b>다 — <b>자르기 시작해야 터진다</b>.</para>
//
//   <para><b>까닭</b>: 테두리를 「첫 줄 전부 → 끝점들 → 마지막 줄 되짚기 → 첫점들」로 둘렀다.
//   가운데 줄을 <b>건너뛴 현(弦)</b>이라, 벽이 ㄷ자이고 날개가 데이라잇까지 뻗으면
//   그 현이 <b>쐐기를 가로질러</b> 삼킨다.</para>
//
//   <para>여기서는 <b>ㄷ자 날개</b>를 만들고, 줄마다 잘리는 자리를 <b>다르게</b> 줘서 그 현을 만든다.</para>
{
    Console.WriteLine("\n== S130 덩이 테두리가 벽 자리를 넘지 않는가(띠) ==");
    static double Ar(IReadOnlyList<Point3> r) => Math.Abs(Shoelace(r));

    // ㄷ자 줄 5개. 줄마다 안쪽으로 1m씩 물러난다. 날개는 y로 30m 뻗는다.
    static List<List<Point3>> RowsU()
    {
        var rows = new List<List<Point3>>();
        for (int i = 0; i < 5; i++)
        {
            double z = 105 + 5 * i, o = i * 1.0;           // 물러남
            var r = new List<Point3>();
            // ※줄은 <b>세 변 모두에서</b> 안쪽으로 물러난다 — 날개도 앞면 높이(y=o)에서 멈춰야 한다.
            //   (처음엔 날개를 y=0까지 내려 그었더니 줄이 <b>제 앞면을 지나쳐</b> 19.4㎡가 어긋났다)
            for (double y = 30; y >= o; y -= 1.0) r.Add(new Point3(0 + o, y, z));     // 왼 날개(위→아래)
            for (double x = 0 + o; x <= 40 - o; x += 1.0) r.Add(new Point3(x, o, z)); // 앞면
            for (double y = o; y <= 30; y += 1.0) r.Add(new Point3(40 - o, y, z));    // 오른 날개
            rows.Add(r);
        }
        return rows;
    }

    // 원지반: 날개 쪽(y↑)과 앞면 양끝(|x−20|↑)이 낮다 → 위 줄일수록 <b>앞면 가운데만</b> 남는다.
    //   그러면 조각의 끝이 「오른 날개 위」에서 「앞면 가운데」로 <b>훌쩍 옮겨</b>,
    //   그 둘을 곧게 이은 현이 <b>ㄷ의 안뜰</b>을 가로지른다 — 현장에서 난 그 일이다.
    //   ※날개 끝만 자르면(처음에 그렇게 짰다) 끝점이 날개를 따라 <b>고르게</b> 내려와
    //     현이 안 생긴다 — 그래서 0.0㎡가 나왔었다.
    Func<double, double, double?> g = (x, y) => 130.0 - y * 0.8 - Math.Abs(x - 20) * 0.3;
    Func<double, double, double?> d = (x, y) => 0.0;

    var lobes = WallTrim.Trim(RowsU(), g, d, 0.5, 0.01, out string lg);
    Console.WriteLine("      S130 " + lg);

    // ★발자국 = <b>줄들이 쓸고 지나간 ㄷ자 띠</b>(맨 바깥 줄 → 맨 안쪽 줄 되짚기).
    //   네모난 겉넓이가 아니다 — 현이 삼키는 곳은 바로 <b>ㄷ의 안뜰</b>이라,
    //   겉넓이로 재면 이 결함이 <b>안 보인다</b>(처음에 그렇게 짜서 0.0㎡가 나왔다).
    var allRows = RowsU();
    var foot = new List<Point3>();
    foreach (var q in allRows[0]) foot.Add(new Point3(q.X, q.Y, 0));
    for (int k = allRows[4].Count - 1; k >= 0; k--)
        foot.Add(new Point3(allRows[4][k].X, allRows[4][k].Y, 0));

    double aChord = 0, aBand = 0; int nBand = 0;
    var bands = new List<IReadOnlyList<Point3>?>();
    foreach (var lb in lobes)
    {
        aChord += Ar(lb.Ring);
        foreach (var b in lb.Bands) { bands.Add(b); aBand += Ar(b); nBand++; }
    }
    var uni = GradingGeometry.RingUnion(bands, 0, out int uh, 1.0);
    double aUni = 0; foreach (var q in uni) { aUni += Ar(q.Ring); foreach (var h in q.Holes) aUni -= Ar(h); }

    // 현 링이 발자국 밖으로 얼마나 나갔나 / 띠 합집합은?
    var footList = new List<IReadOnlyList<Point3>?> { foot };
    double aChordOut = 0;
    foreach (var lb in lobes)
        foreach (var q in GradingGeometry.RingSubtract(lb.Ring, footList, 0, out string _s1, 1.0))
            aChordOut += Ar(q.Ring);
    double aBandOut = 0;
    foreach (var r in uni)
        foreach (var q in GradingGeometry.RingSubtract(r.Ring, footList, 0, out string _s2, 1.0))
            aBandOut += Ar(q.Ring);

    // ★★<b>출하되는 그 길</b>을 그대로 밟는다 — 띠 합집합 → 발자국 안으로 맞춤(RingIntersect).
    var fit = GradingGeometry.RingIntersect(uni, foot, 0, out string fl, 1.0);
    double aFit = 0; foreach (var q in fit) { aFit += Ar(q.Ring); foreach (var h in q.Holes) aFit -= Ar(h); }
    double aFitOut = 0;
    foreach (var r in fit)
        foreach (var q in GradingGeometry.RingSubtract(r.Ring, footList, 0, out string _s3, 1.0))
            aFitOut += Ar(q.Ring);
    // 「발자국 = 벽 + 뚫을 몫」 틈 장부 — 출하 코드가 로그에 찍는 그 식
    var fitFlat = new List<IReadOnlyList<Point3>?>(); foreach (var q in fit) fitFlat.Add(q.Ring);
    var cut = GradingGeometry.RingSubtract(foot, fitFlat, 0, out string _c1, 1.0);
    double aCut = 0; foreach (var q in cut) { aCut += Ar(q.Ring); foreach (var h in q.Holes) aCut -= Ar(h); }
    double gap = Ar(foot) - aFit - aCut;

    Console.WriteLine($"      S130 현 링 {aChord:F1}㎡ · 그중 발자국 <b>밖 {aChordOut:F1}㎡</b>");
    Console.WriteLine($"      S130 띠 {nBand}장 합집합 {aUni:F1}㎡ · 밖 {aBandOut:F1}㎡"
                    + $" → 발자국 맞춤 <b>{aFit:F1}㎡</b> · 밖 <b>{aFitOut:F1}㎡</b> · 발자국 {Ar(foot):F1}㎡");
    Console.WriteLine($"      S130 틈 장부 — 발자국 {Ar(foot):F1} − 벽 {aFit:F1} − 뚫을 몫 {aCut:F1} = <b>{gap:F2}㎡</b>");

    Check("S130 ★이 검사가 <b>뜻이 있다</b> — 다듬기 전엔 발자국 밖으로 나간다", aChordOut > 5.0,
          $"밖 {aChordOut:F1}㎡");
    Check("S130 ★★★<b>발자국 맞춤 뒤엔 밖이 없다</b>", aFitOut < 0.5, $"밖 {aFitOut:F2}㎡");
    Check("S130 ★★★<b>발자국 = 벽 + 뚫을 몫</b>(틈 없음)", Math.Abs(gap) < 1.0, $"{gap:F2}㎡");
    Check("S130 ★★★띠 합집합이 <b>발자국을 안 넘는다</b>", aUni <= Ar(foot) + 5.0,
          $"띠 {aUni:F1}㎡ vs 발자국 {Ar(foot):F1}㎡");
